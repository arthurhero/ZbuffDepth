# Adapted from BTS (https://github.com/cogaplex-bts/bts)
...
class bts(nn.Module):
    ...
    def forward(self, features):
        ...
        #print("skip3 up5",skip3.shape,upconv5.shape)
        ...
        #print("skip2 up4",skip2.shape,upconv4.shape)
        ...
        depth_8x8_scaled_ds = _resize_like(depth_8x8_scaled, skip1, mode='nearest')
        
        upconv3 = self.upconv3(daspp_feat) # H/4
        upconv3 = self.bn3(upconv3)
        #print("skip1 up3",skip1.shape,upconv3.shape)
        ...
        depth_4x4_scaled_ds = _resize_like(depth_4x4_scaled, skip0, mode='nearest')
        
        upconv2 = self.upconv2(iconv3) # H/2
        upconv2 = self.bn2(upconv2)
        #print("skip0 up2",skip0.shape,upconv2.shape)
        ...
        #print("depth_2x2_scaled",depth_2x2_scaled.shape)
        
        upconv1 = self.upconv1(iconv2)
        upconv1 = _resize_like(upconv1,depth_2x2_scaled)
        reduc1x1 = self.reduc1x1(upconv1)
        depth_4x4_scaled = _resize_like(depth_4x4_scaled,depth_2x2_scaled)
        depth_8x8_scaled = _resize_like(depth_8x8_scaled,depth_2x2_scaled)
        #print(upconv1.shape,reduc1x1.shape,depth_4x4_scaled.shape,depth_8x8_scaled.shape)
...
class BtsModel(nn.Module):
    def __init__(self,encoder_name='resnet50_bts',max_depth=100.0):
        super(BtsModel, self).__init__()
        self.encoder = encoder(encoder_name)
        self.decoder = bts(self.encoder.feat_out_channels,max_depth)

    def forward(self, x):
        skip_feat = self.encoder(x)
        depth=self.decoder(skip_feat)
        return depth
    
def _resize_like(inputs, ref,mode='bilinear'):
    _,_,i_h,i_w = inputs.shape
    _,_,r_h,r_w = ref.shape
    if i_h == r_h and i_w == r_w:
        return inputs
    else:
        resized = torch_nn_func.interpolate(inputs, (r_h, r_w), mode=mode)
        return resized
